...
- `upstream_owned`: the defect is in another repository (a vendored `utopia-php/*` library, an SDK, a runtime). Name that repo in `evidence`.
- `needs_human`: fixing it needs a product or ops decision (limits, pricing, data cleanup, a migration on production data).
