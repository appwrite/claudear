...
**Time limit.** The runner stops a live-QA run once it exceeds the smaller of `[agent] timeout_secs` and `[deploy_qa] timeout_secs`, and records it as timed out. That kills the agent CLI process, but not necessarily everything it started, such as a browser or a hung `curl`. Once the CLI is gone, the runner waits a few seconds for anything the CLI started that still holds its output open, then keeps the output read so far, so a run that finished within its limit is still reported. Processing only gives up on the run itself if the runner fails to stop it, and not before the runner has had time past `[deploy_qa] timeout_secs` to return a finished run or to kill and record a timed-out one. Either way a timed-out run's tip is marked `errored`, and the retry manager may run it again up to `[retry] max_retries` times.
